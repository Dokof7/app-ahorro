...
class FinancialSummarySheet implements FromCollection, WithHeadings, WithTitle, WithStyles, ShouldAutoSize
{
    ...
    public function headings(): array
    {
        return ['GRUPO', 'TOTAL AHORRADO', 'F. EMERGENCIA', 'MULTAS COBRADAS', 'PRÉSTAMOS OTORGADOS', 'PRÉSTAMOS RECUPERADOS', 'INTERESES', 'GASTOS BANCARIOS', 'SALDO DISPONIBLE'];
    }
    public function collection(): Collection
    {
        $rows = collect();
        foreach ($this->data['summary'] ?? [] as $item) {
            $rows->push([$item['group']->name, $item['total_savings'], $item['total_emergency'], $item['total_fines'], $item['total_loans_out'], $item['total_loans_recov'], $item['total_interest'], $item['total_bank_expenses'], $item['available_balance']]);
        }
        return $rows;
    }
    public function styles(Worksheet $sheet): array
